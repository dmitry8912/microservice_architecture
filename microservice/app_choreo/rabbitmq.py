...
class RPCHandler:
    ...
    _service_config: dict = None
    _consumer_callback: Callable = None

    def __init__(self, amqp_dsn: str, service_name: str, consumer_callback: Callable, service_config: dict):
    ...
    async def connect(self) -> None:
        logging.info(f"AMQP connecting, config={self._service_config}")
    ...
    async def _consume(self, message: aio_pika.abc.AbstractIncomingMessage) -> None:
        ...
        result = await self._consumer_callback(payload)

        if self._service_config['next_service']:
            next_service = f"{self._service_config['next_service']}_requests"
        else:
            next_service = "api_responses"
        ...
        logging.info(f"AMQP consumer finished message {message.message_id} to {next_service} with id {message.correlation_id}")
        await message.ack()
